...
#include "/work/clas12/users/rojokell/CLAS12Analysis/include/clas12ana/SIDISKinematicsReco.h"
#include "/work/clas12/users/rojokell/CLAS12Analysis/include/clas12ana/Settings.h"
R__LOAD_LIBRARY(libclas12ana.so)
#endif

using namespace std;

int rowanTestpipluspiminus(
//		  const char * hipoFile = "/cache/clas12/rg-a/production/montecarlo/clasdis/fall2018/torus-1/v1/bkg45nA_10604MeV/45nA_job_3301_3.hipo",
		  //const char * hipoFile = "/w/hallb-scshelf2102/clas12/users/rojokell/CLAS12Analysis/data/raw/sample/Out_DIS_pass1_915_920.hipo_skim23.hipo",
...
//		  const char * outputFile = "/work/clas12/users/rojokell/CLAS12Analysis/data/raw/test.root"
          const char * outputFile = "./testoutput.root"
		  )
{
  //---------------
  // Load libraries
  ...
  settings.setElectronBeamEnergy(10.6); // 10.6 for fall2018 10.2 for spring2019
...
//  settings.addPIDforBetarange(22,0.9,1.1); // Beta range for photon
  settings.addPIDforChi2max(211,2.64);        // Pi+ must have abs(chi2pid) < 2.64 [PREVIOUSLY <3, but changed according to Hayward thesis]
  settings.addPIDforChi2max(-211,2.79);       // Pi- must have abs(chi2pid) < 2.79
  ...
  settings.setPostProcessMethod("pipluspiminus"); // Perform pipluspiminus processing
